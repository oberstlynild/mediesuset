...
function validateTicket() {
    ...
    /*ADRESSE*/
    if ((adresse.value)) {
        let letmatch = adresse.value.match(/[a-zA-ZæøåÆØÅ]/g);
        let digmatch = adresse.value.match(/[0-9]/g);

        if (letmatch && digmatch) {
            if (!/^[0-9A-Za-zæøåÆØÅ ]+$/.test(adresse.value)) {
                adresse_label.innerHTML = "Kun bogstaver og numre i adresse!";
    ...
    if (!/^[0-9]+$/.test(postnummer.value)) {
        postnummer_label.innerHTML = "Kun numre i postnummer!";
...
function sendForm(camp_value, email_value, adgangskode_value, gentag_adgangskode_value, navn_value, adresse_value, postnummer_value, by_value, forsendelse_value) {
    console.log(camp_value, email_value, adgangskode_value, gentag_adgangskode_value, navn_value, adresse_value, postnummer_value, by_value, forsendelse_value);
